...
// Get instances (will be available after initialization)
...
router.post('/create-order', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await getDB().collection('payment_orders').doc(order.id).set({
      orderId: order.id,
      teamId,
...
router.post('/verify', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await getDB().collection('payment_orders').doc(razorpay_order_id).update({
...
router.post('/refund', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Store refund record
    await getDB().collection('refunds').doc(refund.id).set({
      refundId: refund.id,
      paymentId,
      teamId,
      amount: amount,
...
router.post('/webhook', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const expectedSignature = createHmac('sha256', process.env.RAZORPAY_WEBHOOK_SECRET || process.env.RAZORPAY_KEY_SECRET || '')
      .update(body)
      .digest('hex');
...
async function handlePaymentCaptured(payment: any) {
  try {
    // Update payment order status
    await getDB().collection('payment_orders').doc(payment.order_id).update({
      status: 'captured',
...
async function handleRefundProcessed(refund: any) {
  try {
    ...
    await getDB().collection('refunds').doc(refund.id).update({
      status: 'processed',
